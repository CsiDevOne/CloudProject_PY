...
class RegistrateUi:
    ...
    def gui_loop(self):
        ...
        self.title = ct.CTkLabel(master=self.main_frame, width=100, height=40, text='Registrate', font=('Roboto', 20),
                                 text_color='#3776ab')
        ...
        self.email_label = ct.CTkLabel(master=self.main_frame, height=10, width=60, text='Email   ', font=('Roboto', 12))
    ...
    def submit_btn_onclick(self):
        ...
        self.socket_obj.send_reg([username, email, password])

        received = self.socket_obj.socket.recv(1024).decode()
        if received == '< success >':
            self.error_output.configure(state='normal', text_color='#298a36')
            self.error_output.insert('1.0', 'registered successfully!')
            self.error_output.configure(state='disabled')

        else:
            self.error_output.configure(state='normal')
            self.error_output.insert('1.0', received)
            self.error_output.configure(state='disabled')
            self.password_input.delete('1.0', 'end')
            self.username_input.delete('1.0', 'end')
